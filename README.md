...
| 🗂️ 文件管理 | 浏览 / 在线编辑 / 新建 / 删除 `data/` 目录下的脚本与配置（自动限制目录，防越权） |
| 📈 系统监控 | 实时 CPU / 内存 / 磁盘 / 网络 占用与 CPU 曲线，每 2 秒刷新 |
| 👥 多用户与权限 | 管理员 / 操作员 / 只读 三角色，基于会话令牌鉴权，后端集中式权限拦截 |
| 🔁 并发任务队列 | 超过「最大并发」的任务自动进入队列等待，不再被丢弃；任务列表显示「运行中 / 排队中」 |
| 📋 任务日志 | 每个任务一键查看历史执行日志与实时输出 |
| 🐳 GitHub Codespaces | 支持在 GitHub 云端一键运行（见 [快速开始](#在-github-codespaces-中运行)） |
...
```bat
双击 start.bat    # 启动面板（启动后自动关闭窗口，进程后台运行）
双击 stop.bat     # 停止面板
```

`start.bat` 启动前会先结束已存在的面板进程（避免重复启动导致端口被占用），并用 `pythonw` 无控制台方式后台运行，运行日志写入 `data/panel.log`。首次运行会自动创建 `.venv` 并安装依赖，随后访问 http://127.0.0.1:5700
...
### 在 GitHub Codespaces 中运行（云端免装环境）

仓库已内置 `.devcontainer` 配置，可在 GitHub 云端一键拉起面板（无需本机装 Python）：

1. 打开仓库页面 → 点击绿色 **Code** → 切到 **Codespaces** 标签 → **Create codespace on main**。
2. 容器启动后会自动安装依赖，并通过 `postStartCommand` 启动面板，5700 端口已配置自动转发。
3. 在弹出的端口面板点 **Open in Browser**（或本地浏览器访问转发的 `https://<codespace>-5700...` 地址）即可使用。

> 说明：Codespaces 按运行时长计费（免费额度每月约 60 小时），闲置会自动休眠；容器重建后 `data/` 里的本地数据会重置，长期数据请改用下方「部署到服务器」。面板内置的 GitHub 自动更新功能在 Codespaces 内同样可用（需先在系统设置里填好你的仓库与 Token）。
...
## 微信对接（内置扫码 / yyb-go 双模式）

左侧导航「微信对接」支持两种登录源，可在页面顶部切换：

### 内置京东扫码（默认，推荐）

- **纯 Python 实现**（`core/jdqr.py`），零外部程序、零编译，开箱即用。
- 前往「京东Cookie」页点「＋ 扫码登录京东」→ 手机**京东 App** 扫码 → 确认登录 → 自动获取 `pt_key/pt_pin` 写入环境变量。
- 支持 Cookie 有效性校验（单个/全部），自动检查任务按 cron 巡检并标记失效账号。
- 京东 Cookie 约 30 天有效，失效后重新扫码即可（京东官方无静默续期接口）。

### 外部 yyb-go（微信扫码）

- **服务配置**：默认本机 IP `127.0.0.1`、端口 `8000`（yyb-go 默认），可自定义；填写 `YYB_API_TOKEN` 作为鉴权。
- **连接状态**：页面实时显示「已连接 / 未连接 / 未启用」圆点，并支持「测试连接」。
- **已登录微信账号**：连接正常时自动列出 yyb-go 中已登录的微信账号（昵称、状态），每 8 秒自动刷新。
...
不想装 yyb-go.exe、又需要给青龙等外部脚本提供 HTTP 取 Cookie 接口时，可直接运行：

```bash
python yyb_py.py                # 默认监听 127.0.0.1:8899
python yyb_py.py --host 0.0.0.0 --port 8899
```

提供与 yyb-go 对齐的 API：`/health`、`/accounts`、`/qr`、`/qr/<sid>/poll`、`/qr/<sid>/confirm`（京东扫码），账号持久化到 `data/yybpy_accounts.json`。
...
### 多用户与角色
- 登录从单账号升级为 **多用户**：账号存于 `users` 表，首次启动自动种子默认账号 `admin / adminadmin`（角色管理员）。
- 三角色：**管理员**（全部权限，含用户管理 / 系统设置 / 备份恢复 / 运维操作）、**操作员**（可建/改/删任务、脚本、订阅、变量、通知、依赖、运行任务，但不能动用户与系统设置）、**只读**（仅浏览，所有写操作被拦截）。
- 后端采用**集中式权限拦截**：`routes/__init__.py` 的 `before_request` 对非只读请求按路径前缀鉴权，未授权返回 403；静态 `api_token`（用于外部脚本调用）一律视为管理员。
- 在「用户管理」页（仅管理员可见）可新建 / 编辑 / 删除用户，分配角色、启用/停用、重置密码。

### 文件管理
- 「文件管理」页可浏览 `data/` 目录（含子目录）、在线编辑、新建文件 / 文件夹、删除。
- 所有路径均被限制在 `data/` 内，含 `..` 或越界的请求会被拒绝，防越权读写。

### 系统监控
- 「系统监控」页实时展示 CPU / 内存 / 磁盘 / 网络 占用，并绘制 CPU 使用率曲线，每 2 秒刷新（依赖 `psutil`）。

### 并发任务队列
- 超过「最大并发任务数」(`max_concurrent`) 的提交会进入队列等待，待空闲槽位释放后自动执行，**不再被丢弃**。
- 任务列表每行显示「运行中 / 排队中」状态；「日志」按钮可查看该任务历史执行输出。
